...
function spreadMapElements(value, key, map, teacherViewMap) {
    ...
    if (teacherViewMap.has(teacherId)) {
        ...
        tempArray.push(`Room No : ${value.room.Room_No} and Time : ${value.time} teaching subject : ${value.subject.Subject_Name} to class ${value.subject.Class_Name}`);
...
function spreadMapElementsLab(value, key, map, teacherViewMap) {
    ...
    if (teacherViewMap.has(teacherId)) {
        let tempArray = teacherViewMap.get(teacherId);
        tempArray.push(`Room No : ${value.lab.Lab_Name} and Time : ${value.time} teaching subject : ${value.subject.Subject_Name} to class ${value.subject.Class_Name}`);
        teacherViewMap.set(teacherId, tempArray);
    } else {
        let tempArray = [];
        tempArray.push(`Room No : ${value.lab.Lab_Name} and Time : ${value.time} teaching subject : ${value.subject.Subject_Name} to class ${value.subject.Class_Name}`);
...
async function teacherViewTimeTable() {
    ...
    try {
        ...
        if (rooms && subjects && labs && teachers) {
            ...
            timeTableRoom.forEach((value, key) => spreadMapElements(value, key, timeTableRoom, TeacherRoomMap));
            timeTableLab.forEach((value , key) =>spreadMapElementsLab(value , key ,timeTableLab , TeacherRoomMap ) )
...
async function getClassViewByID(className) {
    ...
    const roomTT = tt.ttRoom;
    const labTT = tt.ttLab;    
...
module.exports = { generateTimeTableForRoomView , teacherViewTimeTable , getRoomViewByID , getClassViewByID};
